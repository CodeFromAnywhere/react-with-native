import { DivType } from "./Div/Div.type";
import { H2Type } from "./H2/H2.type";
import { StrongType } from "./Strong/Strong.type";
import { ButtonType } from "./Button/Button.type";
import { NavType } from "./Nav/Nav.type";
import { AType } from "./A/A.type";
import { SvgType } from "./Svg/Svg.type";
import { InputType } from "./Input/Input.type";
import { LabelType } from "./Label/Label.type";
import { SpanType } from "./Span/Span.type";
import { PType } from "./P/P.type";
import { LiType } from "./Li/Li.type";
import { UlType } from "./Ul/Ul.type";
import { OlType } from "./Ol/Ol.type";
import { IType } from "./I/I.type";
import { FormType } from "./Form/Form.type";
import { TextAreaType } from "./TextArea/TextArea.type";
import { AsideType } from "./Aside/Aside.type";
import { ImgType } from "./Img/Img.type";
import Nav from "./Nav/Nav";
import Div from "./Div/Div";
import A from "./A/A";
import H2 from "./H2/H2";
import Strong from "./Strong/Strong";
import Button from "./Button/Button";
import Svg from "./Svg/Svg";
import Input from "./Input/Input";
import Img from "./Img/Img";
import Label from "./Label/Label";
import Span from "./Span/Span";
import P from "./P/P";
import Li from "./Li/Li";
import Ul from "./Ul/Ul";
import Ol from "./Ol/Ol";
import Form from "./Form/Form";
import TextArea from "./TextArea/TextArea";
import Aside from "./Aside/Aside";
import I from "./I/I";
export { Div, A, H2, Strong, Button, Svg, Input, Label, Span, P, Li, Ul, Ol, I, Form, TextArea, Aside, Nav, Img, };
export type { DivType, AType, H2Type, StrongType, ButtonType, SvgType, InputType, LabelType, SpanType, PType, LiType, UlType, OlType, IType, FormType, TextAreaType, AsideType, NavType, ImgType, };
//# sourceMappingURL=index.d.ts.map
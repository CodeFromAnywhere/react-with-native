"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getReferencedAssetGetApi = void 0;
var js_util_1 = require("js-util");
var findAbsoluteAssetPathFromReference_1 = require("./util/findAbsoluteAssetPathFromReference");
var serverDownloadReply_1 = require("./util/serverDownloadReply");
/**

Function to get a referenced asset that has been referenced from any db-model or any ts, tsx or md file, anywhere in the project.

Assets should only be exposed if you have access to the model they are referenced in or to the md file they are referenced in.

Exposes all assets like `/asset?path=/text/interests/programming/storage-of-data/best-storage-methods.md&asset=./audio-is-amazing.m4a`


# Security

Assets are only exposed if they are referenced in a markdown file that is accessible to the user. So when looking up the asset, make sure there's a link to it via `[]()` or `![]()` in the markdown that is connected to it in the URL. If not, don't expose it!

This can be used for any ui that has the functions backend available.

The advantage is that we don't need to build it, this works in `dev` mode. there is no asset duplication.

since the name is `getReferencedAssetGetApi` this should be exposed on `getReferencedAsset`

IMPLEMENTATION NOW:

- check referencing file to find the reference.
    - for markdown, typescript: `](./xyz)`
    - for json: `"relativePath": "./xyz"`

- If this occurs, calculate abosolute asset path from referencing file folder + relative path

- If this asset exists, expose it.

*/
var getReferencedAssetGetApi = function (ctx) { return __awaiter(void 0, void 0, void 0, function () {
    var isDownload, projectRelativeReferencingFilePath, referencingFileRelativeAssetPath, _a, absoluteAssetPath, message, isSuccessful;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                isDownload = (0, js_util_1.takeFirst)(ctx.query.isDownload) === "1";
                projectRelativeReferencingFilePath = (0, js_util_1.takeFirst)(ctx.query.projectRelativeReferencingFilePath);
                referencingFileRelativeAssetPath = (0, js_util_1.takeFirst)(ctx.query.referencingFileRelativeAssetPath);
                return [4 /*yield*/, (0, findAbsoluteAssetPathFromReference_1.findAbsoluteAssetPathFromReference)(projectRelativeReferencingFilePath, referencingFileRelativeAssetPath)];
            case 1:
                _a = _b.sent(), absoluteAssetPath = _a.absoluteAssetPath, message = _a.message, isSuccessful = _a.isSuccessful;
                if (!absoluteAssetPath) {
                    return [2 /*return*/, { isSuccessful: isSuccessful, message: message }];
                }
                return [2 /*return*/, (0, serverDownloadReply_1.serverDownloadReply)(absoluteAssetPath, isDownload)];
        }
    });
}); };
exports.getReferencedAssetGetApi = getReferencedAssetGetApi;
//# sourceMappingURL=getReferencedAssetGetApi.js.map